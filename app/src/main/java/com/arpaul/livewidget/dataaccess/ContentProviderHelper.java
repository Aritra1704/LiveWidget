...
import com.arpaul.livewidget.common.ApplicationInstance;
...
public class ContentProviderHelper extends ContentProvider {
    ...
    public static final int HACKTHON                                = 1;
    ...
    static {
        ...
        uriMatcher.addURI(LWCPConstants.PROVIDER_NAME, LWCPConstants.HACKTHON_TABLE_NAME, HACKTHON);
        uriMatcher.addURI(LWCPConstants.PROVIDER_NAME, LWCPConstants.PATH_RELATIONSHIP_JOIN, RELATIONSHIP_JOIN);
    }

    private DataBaseHelper mOpenHelper;

    public static Uri getContentUri(int type) {
        String URL = LWCPConstants.CONTENT + LWCPConstants.PROVIDER_NAME;
        switch (type) {
            case RELATIONSHIP_JOIN:
                URL += "";
                break;
            case HACKTHON:
                URL += "/"+ HackathonDO.TITLE;
                break;
        }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        synchronized (ApplicationInstance.LOCK_APP_DB) {
            ...
            if (_id > 0) {
                returnURI = LWCPConstants.buildLocationUri(_id);
            } else {
                throw new SQLException("Failed to insert row into: " + uri);
            }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        synchronized (ApplicationInstance.LOCK_APP_DB) {
            ...
            switch (uriMatcher.match(uri)) {
                case HACKTHON:
                    table = LWCPConstants.HACKTHON_TABLE_NAME;
                    break;
                case RELATIONSHIP_JOIN:
                    table = selection;
                    selection = null;
    ...
    private String getTableName(Uri uri){
        String table = LWCPConstants.HACKTHON_TABLE_NAME;
    ...
    public String getType(Uri uri) {
        ...
        switch (match) {
            case HACKTHON:
                return LWCPConstants.CONTENT_MULTIPLE_TYPE;
            default: {
                throw new UnsupportedOperationException("Unknown uri: " + uri);
            }
        }
